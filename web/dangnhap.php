...
if (isset($_POST['login'])) {
	$email = $_POST['email'];
	$Password = $_POST['Pass'];
	$check_user = "SELECT * from register WHERE email ='$email' AND password='$Password'";
	$run = mysqli_query($conn, $check_user);

	if (mysqli_num_rows($run)) {
		$_SESSION['email'] = $email; //here session is used and value of $user_email store in $_SESSION. 
		header("Location:http://localhost/huou/project/hotel/");
		exit;
	} else {
...
<head>
	...
	<link href="css/dangnhap.css" rel="stylesheet" type="text/css" media="all" />
	<!-- /css files -->
